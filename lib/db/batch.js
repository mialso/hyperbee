...
const { rebalance } = require('../tree/operation.js')
const { addNode } = require('../tree/add.js')
const { getNode } = require('../tree/get.js')
const { removeNode } = require('../tree/remove.js')
const {
  noop, enc, iteratorPeek, encRange, iteratorToStream
} = require('../util.js')
const { BlockEntry, Key } = require('./block.js')

function bindCas (cas, encoding) {
  if (!cas || !encoding) {
    return
  }
  return async function casFn (prev, next) {
    return cas(prev.final(encoding), next)
  }
}
...
class Batch {
  ...
  async _get (key, encoding) {
    ...
    const rootNode = await this.getRoot(false)
    if (!rootNode) return null

    const predicate = (n) => n.block.isTarget(key)
    const { node, mid, target } = await getNode({ rootNode, key, predicate })
    if (!node) {
      return null
    }
    if (target) {
  ...
  async _put (key, value, encoding, cas) {
    ...
    const casF = bindCas(cas, encoding)

    let rootNode = await this.getRoot(true)
    if (!rootNode) rootNode = TreeNode.create(null)

    const seq = newNode.seq = this.core.length + this.length
    const target = new Key(seq, key)

    const root = await addNode({
      rootNode, newNode, value, target, casF, alwaysDuplicate: this.tree.alwaysDuplicate
    })
    if (!root) {
  ...
  async _del (key, encoding, cas) {
    ...
    const rootNode = await this.getRoot(true)
    if (!rootNode) return this._unlockMaybe()

    const seq = delNode.seq = this.core.length + this.length
    const casF = bindCas(cas, encoding)

    const stack = await removeNode({ rootNode, delNode, key, casF })
    if (!stack) {
      return this._unlockMaybe()
    }
